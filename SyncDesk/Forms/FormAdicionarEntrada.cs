...
{
    public partial class FormAdicionarEntrada : Form
    {
        public string idUser;
        public string nameUser;

        public event Action EntradaAdicionada;
        public FormAdicionarEntrada(string Userid, string UserName)
        {
            InitializeComponent();
            idUser = Userid;
            nameUser = UserName;
                ...
                using (var conn = Database.GetConnection())
                {
                    try
                    {
                        ...
                        using (var cmd = new NpgsqlCommand(query, conn))
                        {
                            ...
                            cmd.Parameters.AddWithValue("@criado_por", int.Parse(idUser));

                            cmd.ExecuteNonQuery();
                            EntradaAdicionada?.Invoke();
                    ...
                    {
                        MessageBox.Show("Erro ao adicionar entrada: " + ex.Message, "Erro", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    }
                }
